...
SERVICE_AREAS_OUT_OF_SCOPE = ["Amstelveen", "Aalsmeer", "Uithoorn"]
...
def create_tree_incident_gdf(incidents_weather_df, trees, grid_gdf):
    ...
    incidents_weather_df = incidents_weather_df[
        ~incidents_weather_df.Service_Area.isin(SERVICE_AREAS_OUT_OF_SCOPE)
    ]

    # create gdf from trees
    tree_gdf = gpd.GeoDataFrame(
        trees, geometry=gpd.points_from_xy(trees["LNG"], trees["LAT"]), crs="EPSG:4326"
    )
    # create gdf from indicents
    incident_gdf = gpd.GeoDataFrame(
        incidents_weather_df,
        geometry=gpd.points_from_xy(incidents_weather_df["LON"], incidents_weather_df["LAT"]),
        crs="EPSG:4326",
    )
    # join with grid gdf
...
def plot_spacial_data(grid_gdf, tree_gdf, incident_gdf, plot_trees=True, plot_incidents=True):
    ...
    fig = px.choropleth_mapbox(
        ...
        zoom=11,
        center={"lat": AMSTERDAM_BBOX[0], "lon": AMSTERDAM_BBOX[1]},
        opacity=0.1,
    )
...
def convert_cat_to_avg(cat_values, delimeter="-"):
    """
...
def enrich_grid_df(grid_gdf, tree_gdf):
...
def main():
    ...
    grid_gdf = gpd.GeoDataFrame(
        grid_df, geometry=gpd.GeoSeries.from_wkt(grid_df["geometry"]), crs="EPSG:4326"
    )
    ...
    tree_gdf, incident_gdf = create_tree_incident_gdf(
        incidents_weather_df=incidents_weather_df, trees=tree_df, grid_gdf=grid_gdf
    )

    # rename categories in new col, in place so only run once
    tree_gdf["boomhoogte"] = [
        MAP_BOOMHOOGTE[klasse] if not klasse is np.nan else np.nan
        for klasse in tree_gdf.boomhoogteklasseActueel.values
    ]
    tree_gdf["stamdiameter"] = [
        MAP_STAMDIAMETER[klasse] if not klasse is np.nan else np.nan
        for klasse in tree_gdf.stamdiameterklasse.values
    ]
    ...
    # sample positives
    positive_samples = create_save_positives(
        incident_gdf=incident_gdf, tree_gdf=tree_gdf, grid_gdf=grid_gdf
    )

    # sample negatives, set according to negative sampling type
    neg_sampler = NegativeSampler(
        has_column="has_tree", has_tree=False, random_dates=True, random_grid=True
    )
    negative_samples = neg_sampler.sample_negatives(
        incidents_weather_df, positive_samples, grid_gdf
    )
    ...
    weather_getter = GetWeather(
        grid_path=GRID_DATA_PATH, samples_path=negatives_path, sleep_time=90
    )

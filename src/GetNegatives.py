...
class NegativeSampler:
    ...
    def verify_sample(
        ...
        grids = incidents[
            (incidents["Date"] >= start_date) & (incidents["Date"] <= end_date)
        ].values

        return False if grid_id not in grids else True

    def sample_random_dates(
        self, num_samples, start_date=RANDOM_START_DATE, end_date=RANDOM_END_DATE
    ):
        dates = []
        hours = []
    ...
    def sample_negatives(self, incidents, positives, grid):
        if self.random_grid:

...
class GetWeather:
    def __init__(self, grid_path, samples_path, num_splits=10, sleep_time=60):
    ...
    def add_weather_data(
        ...
        negative_samples = self.negative_samples.merge(
            grids_gdf[["middle_lat", "middle_lon"]], left_on="grid_id", right_index=True
        )

        negative_samples.rename(columns={"middle_lat": "LAT", "middle_lon": "LON"}, inplace=True)
        columns_order = ["Date", "grid_id", "LAT", "LON"] + [
            col for col in negative_samples.columns if col not in ["Date", "grid_id", "LAT", "LON"]
        ]
    ...
    def get_weather_for_sub(self, df_split):

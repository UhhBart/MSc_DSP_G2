...
class GetPoiDistances:
    ...
    def get_distances(self, city_name="Amsterdam", poi_type="amenity"):
        amenities = self.get_pois(city_name, poi_type)

        self.prio_pois_df = amenities[amenities.amenity.isin(self.poi_types)][
            ["amenity", "geometry"]
        ]
        ...
        if (
            not self.grid_gdf.geometry.is_valid.all()
            or not self.prio_pois_df.geometry.is_valid.all()
        ):
            raise ValueError("Invalid geometries found in one or more GeoDataFrames.")

        # Build KD tree
        poi_coordinates = self.prio_pois_df.geometry.apply(
            lambda geom: (geom.centroid.x, geom.centroid.y)
        ).tolist()
        self.poi_tree = cKDTree(poi_coordinates)

        self.grid_gdf[["distances", "poi_indices"]] = self.grid_gdf.apply(
            self.calculate_distances_within_buffer, axis=1, result_type="expand"
        )

        self.grid_gdf["mean_distance"] = [
            np.mean(x) if isinstance(x, np.ndarray) or isinstance(x, list) else x
            for x in self.grid_gdf["distances"]
        ]

        return self.grid_gdf

    def get_pois(self, city, poi_type):
        """
    ...
    def calculate_distances_within_buffer(self, row):
        buffer = row.buffer
        nearby_pois = self.prio_pois_df[self.prio_pois_df.geometry.within(buffer)]

        if len(nearby_pois) > 0:
            poi_coordinates = nearby_pois.geometry.apply(
                lambda geom: (geom.centroid.x, geom.centroid.y)
            ).tolist()
            poi_tree = cKDTree(poi_coordinates)
            grid_centroid = row.geometry.centroid
            distances, poi_indices = poi_tree.query(
                (grid_centroid.x, grid_centroid.y), k=len(nearby_pois)
            )
